...
int main(int argc, char* argv[])
{
    ...
    if (online)
    {
        ...
        //TEST
        ostringstream test_u_fom_name;
        test_u_fom_name << "u_fom.txt" ;

        ofstream test_u_fom_ofs(test_u_fom_name.str());
        test_u_fom_ofs.precision(16);

        for (int i = 0; i < u_fom.Size(); ++i)
        {
            test_u_fom_ofs << u_fom.Elem(i) << std::endl;
        }

        //TEST
        ostringstream test_u_rom_name;
        test_u_rom_name << "u_rom.txt" ;

        ofstream test_u_rom_ofs(test_u_rom_name.str());
        test_u_rom_ofs.precision(16);

        for (int i = 0; i < u_rom.Size(); ++i)
        {
            test_u_rom_ofs << u_rom.Elem(i) << std::endl;
        }
        ...
        if (myid == 0)
        {
            ...
            //TEST
            cout << "Relative error of ROM displacement (u) at t_final: " << t_final <<
                 " is " << tot_diff_norm_u / tot_u_fom_norm << endl;
            cout << "Relative error of ROM velocity (v) at t_final: " << t_final <<
                 " is " << tot_diff_norm_v / tot_v_fom_norm << endl;

            //TEST
            cout << "unorm diff " << tot_diff_norm_u << endl;
            cout << "unorm fom " << tot_u_fom_norm << endl;
        }
    }
